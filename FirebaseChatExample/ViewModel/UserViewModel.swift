...
class UserViewModel: ObservableObject {
    ...
    @Published var isMessageInitated = false
    ...
    //MARK: - getUserList
    func getUserList() {
        FirebaseManager.shared.fetchUsers(with: "users") { fetchSnapshot in
            DispatchQueue.main.async {
                if let snapshots = fetchSnapshot {
                    self.users = snapshots.compactMap { queryDocumentSnapshot in
                        do {
                            let user = try queryDocumentSnapshot.data(as: User.self)
                            if user.uid != FirebaseManager.shared.getCurrentUser(){
                                return user
                            } else {
                                return nil
                            }
    ...
    //MARK:  Initiate Chat
    func chatInitate(groupName: String, uID: String) {
        var messageUserList = [String]()
        var chat  = [String: Any]()
        if let currentUser = Auth.auth().currentUser {
            messageUserList.append(currentUser.uid)
        }
        messageUserList.append(uID)
        chat["users"] = messageUserList
        chat["groupName"] = groupName
        chat["id"]  = UUID().uuidString
        FirebaseManager.shared.createGroup(with: chat, id: chat["id"] as! String, collection: .messages) { isSuccess in
            self.isMessageInitated = isSuccess
        }
    }
